...
- [x] 3.2 建立 Auth FluentValidation Validators
...
- [x] 5.2 建立管理 API FluentValidation Validators
...
- [x] 8.3 建立 DashboardView（未讀表單數、最近更新統計）
- [x] 8.4 建立 PageManageView（頁面分類 + 子頁面 CRUD + WangEditor + 輪播/Tab 編輯）
- [x] 8.5 建立 NewsManageView（新聞 CRUD + 發佈控制 + 置頂 + WangEditor）
- [x] 8.6 建立 AlbumManageView（相簿 CRUD + 照片上傳 + SortableJS 拖拽排序）
- [x] 8.7 建立 ContactListView（已讀/未讀標記 + 回覆功能）
- [x] 8.8 建立 HomepageSlideView（首頁輪播管理 + 排序）
- [x] 8.9 建立 TrafficManageView（交通資訊 Tab 編輯 + WangEditor）
- [x] 8.10 建立 SettingsView（網站設定 key-value 編輯）
- [x] 8.11 建立 MediaLibraryView（瀏覽、上傳、刪除 + 圖片預覽）
- [x] 8.12 建立 WangEditor 共用元件
- [x] 8.13 驗證所有後台頁面功能正確（npm run build 成功）

## 9. 整合測試與內容遷移

- [x] 9.1 前後端聯調測試（登入 → 後台 CRUD → 前台顯示）
- [x] 9.2 從 PHP 頁面提取文字內容 → 建立 Seed Data
- [x] 9.3 複製 `_images/` 圖片至上傳目錄（部署階段執行）
- [x] 9.4 聯絡流程測試（前台送出 → Email 通知 → 後台查看 → 回覆 → Email 通知訪客）
- [x] 9.5 響應式測試（手機/平板/桌面）
- [x] 9.6 最終編譯驗證（dotnet build + npm run build 無錯誤）
